...
class Group:
    ...
    def __eq__(self, other):
        return (self.id is None or other.id is None or self.id == other.id) and self.name == other.name  # для сравнения не физ адресов объектов, а логич сравнение
                                                                 # (см ролик 4_2 compare_lists)

    def id_or_max(gr): #  вычисляет по группе ключ, используемый для сравнения
        if gr.id: # если у группы есть id
            return int(gr.id) # есть вероятность что id передастся как str
        else:
